...
def get_movie_info(m_id, movieRewindingId):
    ...
    try:
        movieActorList = json_data['programs'][0]['programs']
    ...
    mv_data = json_data['programs'][0]
    # 国家/地区
    try:
        movie_info['regionCategoryName'] = "/" .join([ x for x in json_data['file_area'].split() ])
    except Exception:
        movie_info['regionCategoryName'] = "N/A"
    # 语言
    try:
        movie_info['languageCategoryName'] = "/" .join([ x for x in mv_data['film_language_code'].split() ])
    except Exception:
        movie_info['languageCategoryName'] = "N/A"
    # 字幕
    try:
        movie_info['subtitle'] = '未标注' if len(mv_data['film_caption_code']) == 0 else "/" .join([ x for x in mv_data['film_caption_code'].split() ])
    except Exception:
    ...
    try:
        movie_info['projection_material'] = 'N/A' if json_data['screen_format'] == '' else json_data['screen_format']
    except Exception:
        movie_info['projection_material'] = 'N/A'
    # 画幅比
    try:
        movie_info['frameRatio'] = 'N/A' if mv_data['film_frameratio_code'] == '' else mv_data['film_frameratio_code']
    except Exception:
...
def get_detailed_schedule_info(schedule_data, cinema_list, ts_start_day, ts_end_day):
    ...
    while ts_start_day <= ts_end_day:
        ...
        if month not in month_list:
            ...
            if len(schedule_data) > 0:
                get_detailed_schedule_info(schedule_data, cinema_list, ts_start_day, ts_end_day)
...
def get_schedule_list():
    ...
    for i in xrange(7, 0, -1):
        ...
        if day_num == 1:
            ...
            movie_info_list = get_movie_detailed_info(s_day, cinema_name)
            # add file suffix
